    ...
    *,
    ...
    for _, hotel in all_hotels.iterrows():
        ...
        table_rows_parts.append(
            ...
            f'<td class="hotel-name col-hotel"><a class="open-chart-link hotel-hover-link" href="{chart_href}" '
            f'target="_blank" data-hotel-name="{html_lib.escape(str(hotel_name), quote=True)}">'
            f'{html_lib.escape(str(hotel_name))}</a></td>'
            f'<td class="price col-tight" data-sort-value="{price}"><span class="price-main">{price:.0f} PLN</span>{comeback_cell}</td>'
            f'<td class="col-tight col-w-deal-td" data-sort-value="{deal_score}" title="{deal_title}">'
            ...
            f'<td class="col-tight col-w-ta-td">{ta_html}</td>'
            f'<td class="col-tight col-w-d48-td" data-sort-value="{d48_tbl or 0}"><span class="{delta_class}">{delta_display}</span></td>'
            f'<td class="col-tight col-w-davg-td" data-sort-value="{avg_sort_value}">{avg_display}</td>'
            f'<td class="arrival-hub col-tight" data-sort-value="{html_lib.escape(str(arrival_hub))}">{html_lib.escape(str(arrival_hub))}</td>'
            f'<td class="col-tight col-dates" data-sort-value="{dates}">{dates}</td>'
            f'<td class="col-tight col-w-dur-td col-duration" data-sort-value="{duration}" title="{duration_title}">{duration_display}</td>'
            f'<td class="offer-link-cell col-tight">{offer_link_html}</td></tr>'
        )
