#!/usr/bin/env python3
"""
Генератор исправленного дашборда с решением всех проблем
"""
...
def generate_fixed_dashboard():
    """Генерирует исправленный дашборд"""
...
"""
    ...
    <script>
        ...
        function showHotelChart(hotelName) {{
            ...
            // Загружаем данные для конкретного отеля
            fetch('data/price_alerts_history.json')
                .then(response => response.json())
                .then(data => {{
                    // Фильтруем данные по отелю
                    const hotelData = data.alerts.filter(alert => alert.hotel_name === hotelName);
                    
                    if (hotelData.length > 0) {{
                        // Создаем график на основе данных алертов
                        const chartData = hotelData.map(alert => ({{
                            x: alert.timestamp,
                            y: alert.current_price
...
"""
    ...
    print(f"✅ Исправленный дашборд сгенерирован: index.html")
